...
import (
	...
	"github.com/doitintl/hello/scheduled-tasks/zendesk"
	"github.com/gin-gonic/gin"
)

func ZendeskRenewalsHandler(ctx *gin.Context) {
	const pageSize = 100
	...
	query := fs.Collection("assets").Where("type", "==", common.Assets.Zendesk).Where("properties.subscription.isCommitment", "==", true).Limit(pageSize)
	...
	for {
		...
		for _, docSnap := range docs {
			zendeskRenewals(ctx, fs, docSnap, batch)
		}
...
func zendeskRenewals(ctx *gin.Context, fs *firestore.Client, docSnap *firestore.DocumentSnapshot, batch *firestore.WriteBatch) {
	then := common.ToUnixMillis(time.Now().AddDate(0, 3, 0))

	var asset zendesk.Asset
