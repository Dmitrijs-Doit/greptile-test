...
func (s *StripeService) makeSEPADebitPayment(ctx context.Context, email string, entity *common.Entity, invoiceDocSnap *firestore.DocumentSnapshot, today time.Time, pmID string, amount *int64) error {
	...
	if paymentsDisabled, err := s.stripeDAL.IsPaymentTypeDisabled(ctx, stripe.PaymentMethodTypeSEPADebit); err != nil {
		return err
	...
	if invoice.Symbol != string(fixer.EUR) {
	...
	params := &stripe.PaymentIntentParams{
		...
		PaymentMethodTypes:        []*string{stripe.String(string(stripe.PaymentMethodTypeSEPADebit))},
		Amount:                    amount,
