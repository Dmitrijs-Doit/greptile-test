...
func (s *StripeService) makeACHPayment(ctx context.Context, email string, entity *common.Entity, invoiceDocSnap *firestore.DocumentSnapshot, today time.Time, pmID string, amount *int64) error {
	...
	if paymentsDisabled, err := s.stripeDAL.IsPaymentTypeDisabled(ctx, stripe.PaymentMethodTypeUSBankAccount); err != nil {
		return err
	...
	if invoice.Symbol != string(fixer.USD) {
	...
	var (
		openInvoiceDebit = int64(math.Round(openInvoice.Debit * 100))
		invoiceBalance   = int64(math.Round(invoice.Debit * 100))
	)
	...
	if strings.HasPrefix(pm.ID, "ba_") {
		// Backwards compatibility for old (ACH) bank accounts payment methods
		// The ID has a prefix of "ba_", and they don't have a mandate configured
		params.SetupFutureUsage = stripe.String(string(stripe.PaymentIntentSetupFutureUsageOffSession))
		params.MandateData = &stripe.PaymentIntentMandateDataParams{
			CustomerAcceptance: &stripe.PaymentIntentMandateDataCustomerAcceptanceParams{
				Type: stripe.String(string(stripe.MandateCustomerAcceptanceTypeOffline)),
			},
		}
	} else {
		// New US Bank account payment methods starts with "pm_"
		// and already have a mandate configured
