...
func (s *StripeService) makeBACSDebitPayment(ctx context.Context, email string, entity *common.Entity, invoiceDocSnap *firestore.DocumentSnapshot, today time.Time, pmID string, amount *int64) error {
	...
	if invoice.Symbol != string(fixer.GBP) {
	...
	params := &stripe.PaymentIntentParams{
		...
		PaymentMethodTypes:        []*string{stripe.String(string(stripe.PaymentMethodTypeBACSDebit))},
		Amount:                    amount,
