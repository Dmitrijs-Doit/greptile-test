...
const (
	...
	AwsOrgPrefix string = "aws-org/"
)

const comma string = ", "

const (
	eksRawDataTableString string = `WITH
	raw_data AS (
		{union_select_start}
			SELECT *,
...
func projectLabelsString(filterOUAWSTags bool) string {
	whereClause := ""
	if filterOUAWSTags {
		whereClause = fmt.Sprintf("AND NOT STARTS_WITH(label.key, '%s')", AwsOrgPrefix)
	}
