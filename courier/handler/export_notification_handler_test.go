...
func TestCourierHandler_ExportNotificationToBQHandler(t *testing.T) {
	email := "test@doit.com"
	...
	type args struct {
		body *domain.ExportNotificationReq
	}

	startDate := "2023-01-02"

	req := domain.ExportNotificationReq{
		StartDate:      &startDate,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			respond := h.ExportNotificationToBQHandler(ctx)
			...
			if (respond != nil) != tt.wantErr {
				t.Errorf("Courier.ExportNotificationToBQHandler() error = %v, wantErr %v", respond, tt.wantErr)
				return
			}
		})
	}
