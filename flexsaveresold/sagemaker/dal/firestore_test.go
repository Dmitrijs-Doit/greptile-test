...
func Test_dal_collection(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			want:   "projects//databases//documents/integrations/flexsave/configuration-sagemaker",
		},
	}
...
func Test_dal_Get(t *testing.T) {
	...
	tests := []struct {
		name    string
		fields  fields
		want    *sageMakerIface.FlexsaveSageMakerCache
		wantErr error
		on      func(f *fields)
	}{
		{
					...
					Return(func() iface.DocumentSnapshot {
						...
						snap.On("DataTo", mock.Anything).Return(nil).
							Run(func(args mock.Arguments) {
								arg := args.Get(0).(*sageMakerIface.FlexsaveSageMakerCache)
...
func Test_dal_AddReasonCantEnable(t *testing.T) {
	...
	}{
		{
			on: func(f *fields) {
				f.documentsHandler.On("Get", testutils.ContextBackgroundMock, mock.AnythingOfType("*firestore.DocumentRef")).
					Return(func() iface.DocumentSnapshot {
						...
						snap.On("DataTo", mock.Anything).Return(nil).
							Run(func(args mock.Arguments) {
								arg := args.Get(0).(*sageMakerIface.FlexsaveSageMakerCache)
								arg.ReasonCantEnable = []sageMakerIface.FlexsaveSageMakerReasonCantEnable{"foo"}
							}).Once()
				...
				f.documentsHandler.On(
					...
					map[string]interface{}{"reasonCantEnable": []sageMakerIface.FlexsaveSageMakerReasonCantEnable{"foo", "no_billing_table"}}, firestore.MergeAll).
					Return(nil, nil).Once()
			},
			name:    "updates correctly",
			wantErr: nil,
	...
	for i := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := d.AddReasonCantEnable(context.Background(), "kawa", domain.FlexsaveSageMakerReasonCantEnableNoBillingTable)
			assert.Equal(t, tt.wantErr, err)
		})
	}
}
...
func Test_dal_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			on: func(f *fields) {
				f.documentsHandler.On("Create", testutils.ContextBackgroundMock, mock.Anything, sageMakerIface.FlexsaveSageMakerCache{
