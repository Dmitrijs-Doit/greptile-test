...
func (s service) CreateSavingsHistory(ctx context.Context, customerID string, now time.Time, numberOfMonths int) (map[string]iface.MonthSummary, error) {
	...
	go s.bigQueryService.GetCustomerOnDemand(params, rdsOnDemandQuery, onDemandChan, errChan)
	go s.bigQueryService.GetCustomerSavings(params, rdsSavingsQuery, savingsMonthlyChan, errChan)
