...
// BillingUpdateFirestore is used to interact with Flexsave standalone update information stored on Firestore.
...
func (d *BillingUpdateFirestore) GetRef(ctx context.Context, id string) *firestore.DocumentRef {
	return d.firestoreClientFun(ctx).Collection(BillingUpdateCollection).Doc(id)
}

func (d *BillingUpdateFirestore) BillingUpdateCollection(ctx context.Context) *firestore.CollectionRef {
	return d.firestoreClientFun(ctx).Collection(BillingUpdateCollection)
}
